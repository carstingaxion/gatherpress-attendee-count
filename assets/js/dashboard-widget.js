( function ( $ ) {
	...
	$( document ).ready( function () {
		...
		$( '#gatherpress_attendee_count_widget' ).on(
			'click',
			...
			function ( e ) {
				...
				const saveEdit = function () {
					...
					if ( newValue === '' || newValue === '0' || newValue < 1 ) {
						alert(
							'Please enter a valid attendee count (minimum 1).'
						);
						return;
					}
					...
					$.ajax( {
						...
						success( response ) {
							if ( response.success ) {
								...
								const eventItem = container.closest(
									'.gatherpress-event-item'
								);
								eventItem.fadeOut( 300, function () {
									$( this ).remove();

									const remainingEvents = $(
										'#gatherpress-events-list li'
									).length;
									if ( remainingEvents === 0 ) {
										$(
											'#gatherpress_attendee_count_widget .inside'
										).html(
