(function ($) {
	...
	$(document).ready(function () {
		var canEdit = gatherpressAttendeeCountListTable.canEdit;
		...
		$('.column-gatherpress_attendee_count').on('click', '.gatherpress-count-value', function (e) {
			e.preventDefault();

			var valueSpan = $(this);
			var container = valueSpan.parent();
			var currentValue = valueSpan.data('value');
			...
			var saveEdit = function () {
				...
				if (newValue === '' || newValue === '0' || newValue < 1) {
					...
					if (currentValue !== '—') {
						saveBtn.prop('disabled', true).text('Clearing...');
						...
						$.ajax({
							...
							success: function (response) {
								if (response.success) {
									...
									valueSpan.html('—');
									valueSpan.addClass('gatherpress-empty');
									...
									alert(response.data.message || 'Failed to clear attendee count.');
									container.removeClass('gatherpress-saving');
				...
				$.ajax({
					...
					success: function (response) {
						if (response.success) {
							...
							valueSpan.html('<span>' + newValue + '</span>');
							...
							setTimeout(function () {
								...
								valueSpan.animate({ color: '#2c3338' }, 1000);
							}, 2000);
						} else {
